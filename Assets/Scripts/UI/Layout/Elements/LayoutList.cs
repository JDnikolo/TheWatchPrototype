...
namespace UI.Layout.Elements
{
	...
	public sealed class LayoutList : LayoutElement, ILayoutControllingParent, ILayoutExplicit, ILayoutExplicitCallbackImplementer
	{
		...
		private readonly Dictionary<ILayoutElement, Node> m_nodes = new();
		...
		public void OnSelectingNewHierarchy(ILayoutElement newElement, ILayoutElement oldElement)
		{
			...
			else m_selectedNode = Find(oldElement);
		...
		public void AddFirst(ILayoutElement element)
		{
			...
			m_nodes.Add(element, m_elements.AddFirst(element));
			element.Parent = this;
		...
		public void AddLast(ILayoutElement element)
		{
			...
			m_nodes.Add(element, m_elements.AddLast(element));
			element.Parent = this;
		...
		public void Remove(Node node)
		{
			...
			if (node.Value != null) m_nodes.Remove(node.Value);
		...
		private Node Find(ILayoutElement element) => m_nodes.GetValueOrDefault(element);
		...
		{
			base.OnPrewarm();
			this.CollectChildren<LayoutElementBase, ILayoutElement>(m_elements);
			SetFirstNode();
		}
#if UNITY_EDITOR
		...
		private HierarchyTester<LayoutElementBase> m_tester;
		...
		private void SetElementParents()
		{
			...
			{
				if (!parent.GetChild(childIndex).GetComponent<LayoutElement>()) continue;
				firstIndex = childIndex;
				break;
			}

			if (firstIndex < 0) return;
			var firstElement = parent.GetChild(firstIndex).GetComponent<LayoutElement>();
			var lastIndex = -1;
			...
			var lastElement = parent.GetChild(lastIndex).GetComponent<LayoutElement>();
			...
			{
				var element = parent.GetChild(childIndex).GetComponent<LayoutElement>();
				element.SetManagedParent(this);
				while (childIndex <= lastIndex)
				{
					childIndex += 1;
					if (!parent.GetChild(childIndex).TryGetComponent(out LayoutElement nextElement)) continue;
					switch (direction)
					{
						case Axis.Horizontal:
